...
#define G3_LIB_PEPROTOVERSION_ID        ((uint32_t)0x3201)  /**< @brief G3_LIB Attribute peProtocolVersion: store the value of the bandplan used. It can be #G3_LIB_G3PLC_CENA or #G3_LIB_G3PLC_CENB or #G3_LIB_G3PLC_ARIB or #G3_LIB_IEEE_1901_2_FCC #G3_LIB_G3PLC_FCC */
...
#define G3_LIB_G3PLC_FCC_LOW         ((uint8_t)0x04)  /**< @brief Library configuration value for ITU-T G.9903 - FCC LOW bandplan */
#define G3_LIB_G3PLC_FCC_HIGH        ((uint8_t)0x05)  /**< @brief Library configuration value for ITU-T G.9903 - FCC HIGH bandplan */
...
#define G3_LIB_PEEVENTINDICATION_MACUPDATE 	   		((uint64_t)0x0000000000080000)  /**< @brief Bitmask definition for the event notification */
#define G3_LIB_PEEVENTINDICATION_MEMLEAK 	   		((uint64_t)0x0000000000100000)  /**< @brief Bitmask definition for the event notification */
#define G3_LIB_PEEVENTINDICATION_TIMERERR 	   		((uint64_t)0x0000000000200000)  /**< @brief Bitmask definition for the event notification */
#define G3_LIB_PEEVENTINDICATION_PHYQUALITY_RX_RF   ((uint64_t)0x0000000000400000)  /**< @brief Bitmask definition for the event notification */
#define G3_LIB_PEEVENTINDICATION_PHYQUALITY_TX_RF   ((uint64_t)0x0000000000800000)  /**< @brief Bitmask definition for the event notification */
#define G3_LIB_PEEVENTINDICATION_ADPDATAFORWARD 	((uint64_t)0x0000000001000000)  /**< @brief Bitmask definition for the event notification */
...
typedef struct
{
    uint16_t DstAddr;       /**< @brief Short address of the final destination node */
    uint16_t NextHop;       /**< @brief Short address of the next hop */
    uint16_t RouteCost;     /**< @brief The cumulative Route Cost towards the destination */
    uint8_t  HopCount : 4;  /**< @brief Bit 0..3: Hop count */
    uint8_t  WeakLink : 4;  /**< @brief Bit 4..7: Weak link count */
    uint16_t ValidTime;     /**< @brief The remaining valid time in minutes for this entry (0 = entry is no more valid)  */
    uint8_t  MediaType;     /**< @brief The medium to be used to transmit to the Next Hop */
...
#define G3_LIB_MAX_ATTR_LEN  		(sizeof(G3_LIB_PIBTypesStructsType)) /**< @brief Maximum dimension of one value in the PIB  */
...
typedef struct
{
    G3_LIB_PIB_ID_t  attribute_id;               /**< @brief The ID+Indexof the attribute  */
...
#define G3_LIB_PIB_SIZE(attr_len)	(sizeof(G3_LIB_PIB_t) - G3_LIB_MAX_ATTR_LEN + attr_len)
