...
void g3_app_attrib_tbl_init(void)
{
	...
	if ((working_plc_mode == PLC_MODE_IPV6_BOOT) || (working_plc_mode == PLC_MODE_IPV6_ADP))
	{
		...
		const uint8_t active_gmk_index          = DEFAULT_GMK_INDEX;
		...
		g3_attrib_tbl_add(MAC_KEYTABLE_ID,          0, sizeof(default_gmk),     	default_gmk);
		...
		/* Suggested attributes */
		g3_attrib_tbl_add(ADP_COORDSHORTADDRESS_ID, 0, sizeof(coord_addr),			coord_addr);
