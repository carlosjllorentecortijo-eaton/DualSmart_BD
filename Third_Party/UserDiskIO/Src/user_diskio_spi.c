...
#define SD_CARD_PRESENT()	(HAL_GPIO_ReadPin(SDC_DETECT_GPIO_Port, SDC_DETECT_Pin) == GPIO_PIN_RESET)
#define SD_CARD_ABSENT()	(HAL_GPIO_ReadPin(SDC_DETECT_GPIO_Port, SDC_DETECT_Pin) == GPIO_PIN_SET)
...
)
{
	...
	BYTE rxDat;
	result = HAL_SPI_TransmitReceive(&SD_SPI_HANDLE, &dat, &rxDat, 1, 50);
...
)
{
	...
	if (send_cmd(CMD0, 0) == 1)
	{
		...
		SPI_Timer_On(configTICK_RATE_HZ);	/* Initialization timeout = 1 sec */
...
)
{
	...
	if (Stat & STA_NODISK) return RES_ERROR;	/* Is card existing in the socket? */
