...
uint8_t modbus_queue_buffer[ MAX_TELEGRAMS * sizeof(modbus_t) ];
StaticQueue_t modbus_queue_control_block;
const osMessageQueueAttr_t modbus_queue_attributes = {
  .name = "ModbusQueue",
  .cb_mem = &modbus_queue_control_block,
  .cb_size = sizeof(modbus_queue_control_block),
  .mq_mem = &modbus_queue_buffer,
  .mq_size = sizeof(modbus_queue_buffer)
};

uint32_t modbus_task_stack[ 128 ];
StaticTask_t modbus_task_control_block;
const osThreadAttr_t modbus_task_attributes = {
  .name = "ModbusTask",
  .stack_mem = &modbus_task_stack[0],
  .stack_size = sizeof(modbus_task_stack),
  .cb_mem = &modbus_task_control_block,
  .cb_size = sizeof(modbus_task_control_block),
  .priority = osPriorityNormal,
};

//Semaphore to access the Modbus Data
StaticSemaphore_t modbus_semaphore_control_block;
const osSemaphoreAttr_t modbus_semaphore_attributes = {
  .name = "ModbusSem",
  .cb_mem = &modbus_semaphore_control_block,
  .cb_size = sizeof(modbus_semaphore_control_block),
};

StaticTimer_t modbus_timer_timeout_control_block;
const osTimerAttr_t modbus_timer_timeout_attributes = {
  .name = "TimerTimeout",
  .cb_mem = &modbus_timer_timeout_control_block,
  .cb_size = sizeof(modbus_timer_timeout_control_block),
};

StaticTimer_t modbus_timer_t35_control_block;
const osTimerAttr_t modbus_timer_t35_attributes = {
  .name = "TimerT35",
  .cb_mem = &modbus_timer_t35_control_block,
  .cb_size = sizeof(modbus_timer_t35_control_block),
};
...
void TimerT35Callback(TimerHandle_t *timer_handle);
void TimerTimeoutCallback(TimerHandle_t *timer_handle);
...
void ModbusInit(modbusHandler_t * modH)
{
	if (numberHandlers < MAX_M_HANDLERS)
	{
		...
		if(modH->uModbusType == MB_SLAVE)
		{
			...
			modH->modbus_taskHandle = osThreadNew(StartTaskModbusSlave, modH, &modbus_task_attributes);
		}
		else if (modH->uModbusType == MB_MASTER)
		{
			//Create Modbus task Master  and Queue for telegrams
			modH->modbus_taskHandle = osThreadNew(StartTaskModbusMaster, modH, &modbus_task_attributes);

			modH->TimerTimeout = xTimerCreate("TimerTimeout", modH->u16timeOut, pdFALSE, NULL, (TimerCallbackFunction_t) TimerTimeoutCallback);

			if (modH->TimerTimeout == NULL)
			{
				Error_Handler(); //error creating timer, check heap and stack size
			}

			modH->QueueTelegramHandle = osMessageQueueNew(MAX_TELEGRAMS, sizeof(modbus_t), &modbus_queue_attributes);

			if (modH->QueueTelegramHandle == NULL)
			{
				Error_Handler(); //error creating queue for telegrams, check heap and stack size
			}
		}
		else
		{
			Error_Handler(); //Error Modbus type not supported choose a valid Type
		}

		if  (modH->modbus_taskHandle == NULL)
		{
			Error_Handler(); //Error creating Modbus task, check heap and stack size
		}

		modH->TimerT35 = xTimerCreate("TimerT35", T35, pdFALSE, NULL, (TimerCallbackFunction_t) TimerT35Callback);

		if (modH->TimerT35 == NULL)
		{
			Error_Handler(); //Error creating the timer, check heap and stack size
		}

		modH->ModBusSphrHandle = osSemaphoreNew(1, 1, &modbus_semaphore_attributes);

		if(modH->ModBusSphrHandle == NULL)
		{
			Error_Handler(); //Error creating the semaphore, check heap and stack size
		}
	...
	{
		Error_Handler(); //error no more Modbus handlers supported
	}
}
...
void ModbusStart(modbusHandler_t * modH)
{
	...
	{
		Error_Handler(); //ERROR select the type of hardware
	}

#if !ENABLE_MODBUS_USART_DMA
	if (modH->xTypeHW == USART_HW_DMA)
	{
		Error_Handler(); //ERROR To use USART_HW_DMA you need to enable it in the ModbusConfig.h file
	}
#endif
	...
	{
		Error_Handler(); //error Define the DATA pointer shared through Modbus
	}
	...
	if (modH->xTypeHW == USART_HW_DMA)
	{
		...
		{
			Error_Handler(); //error UART initialization
		}
	...
	else
	{
		...
		if(HAL_UART_Receive_IT(modH->port, &modH->dataRX, sizeof(modH->dataRX)) != HAL_OK)
		{
			Error_Handler(); //error UART initialization
		}
#if ENABLE_MODBUS_USART_DMA
	...
	{
		Error_Handler(); //error Master ID must be zero
	}

	if (modH->u8id == 0 && modH->uModbusType == MB_SLAVE)
	{
		Error_Handler(); //error Slave ID must not be zero
	}
...
/**
 * @brief De-initialization for a Master/Slave.
 * @param modH   modbus handler
 */
void ModbusDeinit(modbusHandler_t * modH)
{
	...
	{
		//Deletes Modbus task
		if (osThreadTerminate(modH->modbus_taskHandle) != osOK)
		{
			Error_Handler();
		}
		...
		{
			//Deletes timeout timer and queue for telegrams
			xTimerDelete(modH->TimerTimeout, 0);

			if (osMessageQueueDelete(modH->QueueTelegramHandle) != osOK)
			{
				Error_Handler();
			}
		}

		xTimerDelete(modH->TimerT35, 0);

		if (osSemaphoreDelete(modH->ModBusSphrHandle) != osOK)
		{
			Error_Handler();
		}
...
#if MAX_M_HANDLERS > 1
		/* Handlers flow */
		for (int i = 0; i < (numberHandlers - 1); i++)
		{
			if (mHandlers[i] == modH)
			{
...
void TimerT35Callback(TimerHandle_t *timer_handle)
{
	...
	for (int i = 0; i < numberHandlers; i++)
	{
		...
		if ((TimerHandle_t *)mHandlers[i]->TimerT35 == timer_handle)
		{
			if (mHandlers[i]->uModbusType == MB_MASTER)
			{
				xTimerStop(mHandlers[i]->TimerTimeout, 0);
			}

			xTaskNotify(mHandlers[i]->modbus_taskHandle, 0, eSetValueWithOverwrite);
		}
	}
}
...
void TimerTimeoutCallback(TimerHandle_t *timer_handle)
{
	...
	for (int i = 0; i < numberHandlers; i++)
	{
		if ((TimerHandle_t*) mHandlers[i]->TimerTimeout ==  timer_handle)
		{
			xTaskNotify(mHandlers[i]->modbus_taskHandle, ERR_TIME_OUT, eSetValueWithOverwrite);
		}
	}
}
...
void StartTaskModbusSlave(void *argument)
{
	...
	for(;;)
	{
		...
		int32_t u8exception = validateRequest(modH);
		if (u8exception > 0)
		{
		...
		modH->i8lastError = 0;
		osSemaphoreAcquire(modH->ModBusSphrHandle , portMAX_DELAY); //before processing the message get the semaphore
...
bool ModbusQuery(modbusHandler_t * modH, modbus_t telegram)
{
	bool success = true;
...
int8_t SendQuery(modbusHandler_t *modH ,  modbus_t telegram )
{
	...
	else if (modH->i8state != COM_IDLE)
	{
		error = ERR_POLLING ;
	}
	else if ((telegram.u8id == 0) || (telegram.u8id > 247))
	{
...
void StartTaskModbusMaster(void *argument)
{
	...
	for(;;)
	{
		...
		xTimerStop(modH->TimerTimeout, 0); // cancel timeout timer
...
static void sendTxBuffer(modbusHandler_t *modH)
{
	...
	if (modH->uModbusType == MB_MASTER)
	{
		xTimerReset(modH->TimerTimeout, 0);
	}
	 ...
	 */
#if defined(STM32H7)  || defined(STM32F3) || defined(STM32L4) || defined(STM32G0) || defined(STM32G4)
